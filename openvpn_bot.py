...
async def newclient(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        subprocess.run(f"cd {EASYRSA_DIR} && ./easyrsa gen-req {clientname} nopass", shell=True, check=True)
        subprocess.run(f"cd {EASYRSA_DIR} && echo yes | ./easyrsa sign-req client {clientname}", shell=True, check=True)
